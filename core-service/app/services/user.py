...
from app.config.security import hash_password, verify_password, generate_token, str_encode, str_decode, load_user
from app.models.user import User, UserToken
from app.config.security import is_password_strong_enough
from app.services.email import send_account_verification_email, send_account_activation_confirmation_email, send_password_reset_email, send_password_reset_confirmation_email
...
async def activate_user_account(data, session, background_tasks):
    ...
    if not token_valid:
        raise HTTPException(status_code=400, detail="This link is either expired or invalid")
...
async def get_login_token(data, session):
    ...
    if not user.verified_at:
        raise HTTPException(status_code=400, detail="Account is not verified yet")
...
async def reset_user_password(data, background_tasks, session):
    ...
    # Notify user that password has been updated by sending an email or a push notification
    await send_password_reset_confirmation_email(user, background_tasks)
